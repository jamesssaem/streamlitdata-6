...
def addESPrediction(data, ax, pred_ndays):
    ...
    model = ExponentialSmoothing(ser, seasonal='add',seasonal_periods=5).fit()    # 승법모형. 학습 완료.
    past = ser.iloc[-5:]
    predicted = model.predict(start= n, end=n+pred_ndays-1) # 승법 모형 예측.
...
# 캔들 차트 위에 예측을 그주는 함수.
def plotChartV3(data, pred_ndays):
...
plotChartV3(df, st.session_state['pred_ndays'])
''
''
